...
def IK_simple_open_chain(chain_a,chain_b,dir_graph):
...
chain_b.append(['jnt_b0', np.array([[ 1, 0, 0],
                                    [ 0, 1, 0],
                                    [ 0, 0, 1]]),
                          np.array([0,-0.2,0])])
chain_b.append(['jnt_b1', np.array([[ 1, 0, 0],
                                    [ 0, 1, 0],
                                    [ 0, 0, 1]]),
                          np.array([0,0,-0.2])])
chain_b.append(['jnt_b2', np.array([[ 1, 0, 0],
                                    [ 0, 1, 0],
                                    [ 0, 0, 1]]),
                          np.array([0,-0.2,-0.2])])
chain_b.append(['jnt_b3', np.array([[ 1, 0, 0],
                                    [ 0, 1, 0],
                                    [ 0, 0, 1]]),
                          np.array([0.5,0,0])])
chain_b.append(['jnt_b4', np.array([[ 1, 0, 0],
                                    [ 0, 1, 0],
                                    [ 0, 0, 1]]),
                          np.array([0,-0.5,0])])
...
ori_a, pos_a = FK_local2global(copy.deepcopy(chain_a),dir_graph)

ori_b, pos_b = FK_local2global(copy.deepcopy(chain_b),dir_graph)
...
fig = plt.figure(1)
ax = fig.add_subplot(111, projection='3d')
fig.suptitle('Simple open kinematic chain a, plotted using forward kinematics', fontsize=12)

dirxglobal=[1,0,0]
diryglobal=[0,1,0]
dirzglobal=[0,0,1]

ax.quiver(-1,-1,-1,dirxglobal[0],dirxglobal[1],dirxglobal[2],length=0.5,color='r')
ax.quiver(-1,-1,-1,diryglobal[0],diryglobal[1],diryglobal[2],length=0.5,color='g')
ax.quiver(-1,-1,-1,dirzglobal[0],dirzglobal[1],dirzglobal[2],length=0.5,color='b')
...
# Plot simple chains a and b  --> Plot b without local joint orientations
# ----------------------------------------------------------------------------------------------------------------
fig = plt.figure(2)
ax = fig.add_subplot(111, projection='3d')
fig.suptitle('Simple open kinematic chains a & b', fontsize=12)
...
# Apply inverse kinematics to map simple chain a to chain b
# ----------------------------------------------------------------------------------------------------------------
reori_a, repos_a, Euler_axes = IK_simple_open_chain(copy.deepcopy(chain_a),copy.deepcopy(chain_b),dir_graph)
# ----------------------------------------------------------------------------------------------------------------



# Plot simple chain a repositioned
# ----------------------------------------------------------------------------------------------------------------
fig = plt.figure(3)
ax = fig.add_subplot(111, projection='3d')
fig.suptitle('Kinematic chain a repositioned', fontsize=12)
...
chain_a.append(['jnt_a0', np.array([[ 0.1466, 0.7779, -0.6111],
                                    [ -0.04568, -0.6118, -0.7897],
                                    [ -0.9881, 0.1437, -0.05418]]),
                          np.array([0,0,0])])
chain_a.append(['jnt_a1', np.array([[ -0.531, 0.7242, -0.4399],
                                    [ -0.267, -0.6357, -0.7243],
                                    [ -0.8042, -0.2672, 0.5309]]),
                          np.array([-0.1,-0.1,-0.1])])
chain_a.append(['jnt_a2', np.array([[ -0.1898, 0.7854, -0.5892],
                                    [ -0.395, -0.6104, -0.6865],
                                    [ -0.8989, 0.1024, 0.4261]]),
                          np.array([0.2,-0.2,0])])
chain_a.append(['jnt_a3', np.array([[ 0.8989, -0.1024, -0.4261],
                                    [ -0.395, -0.6104, -0.6865],
                                    [ -0.1898, 0.7854, -0.5892]]),
                          np.array([0.1,0,0.1])])
chain_a.append(['jnt_a4', np.array([[ -0.6784, -0.2447, 0.6928],
                                    [ -0.4144, -0.6512, -0.6358],
                                    [ 0.6067, -0.7184, 0.3404]]),
                          np.array([0,0.1,0.1])])
chain_a.append(['jnt_a5', np.array([[ 0.3584, 0.5254, -0.7717],
                                    [ 0.4144, 0.6512, 0.6358],
                                    [ 0.8366, -0.5476, 0.01571]]),
                          np.array([0.1,0.2,0])])
chain_a.append(['jnt_a6', np.array([[ -0.3819, 0.8207, -0.425],
                                    [ -0.2589, -0.5365, -0.8032],
                                    [ -0.8872, -0.1967, 0.4174]]),
                          np.array([0,0,-0.1])])
chain_a.append(['jnt_a7', np.array([[ -0.3384, 0.8237, -0.455],
                                    [ -0.688, -0.5464, -0.4776],
                                    [ -0.642, 0.1514, 0.7516]]),
                          np.array([0,0.2,-0.1])])
chain_a.append(['jnt_a8', np.array([[ 0.1306, 0.5971, 0.7915],
                                    [ -0.4515, 0.7465, -0.4888],
                                    [ -0.8827, -0.2935, 0.367]]),
                          np.array([0.1,0.1,-0.1])])
chain_a.append(['jnt_a9', np.array([[ -0.4515, 0.7465, -0.4888],
                                    [ -0.1306, -0.5971, -0.7915],
                                    [ -0.8827, -0.2935, 0.367]]),
                          np.array([-0.1,-0.1,0])])
chain_a.append(['jnt_a10', np.array([[ -0.02267, 0.9877, -0.1547],
                                     [ 0.9976,  0.03246, 0.06109],
                                     [ 0.06537,  -0.153, -0.9861]]),
                           np.array([0,0,-0.1])])
